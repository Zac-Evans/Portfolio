...
var AnimatedPhrases = React.createClass({
  ...
  isLastPhrase: function () {
    ...
    return (
      this.state.currentPhrase === this.state.phraseCount - 1 &&
    ...
    if (phrase.hasOwnProperty("name")) {
      ...
      if (isVisible) {
        ...
        if (phrase.hasOwnProperty("url") && phrase.hasOwnProperty("id")) {
          return (
            <a className={classes} key={index} href={phrase.url}>
              {splitPhrase}
            </a>
          );
        } else {
          return (
            <b className={classes} key={index}>
              {splitPhrase}
            </b>
          );
        }
  ...
  singleLetters: function (phrase) {
    ...
    var newLetters = letters.map(function (letter, index) {
      return (
        <i className={currentLetter >= index ? "in" : "out"} key={index}>
          {letter}
        </i>
      );
    });
  ...
  render: function () {
    ...
    return (
      <h1 className="animated-phrase letters type">
        ...
        <span className="word-wrapper waiting">{phrases}</span>
      </h1>
    );
  },
});
